...
var children_client = function(public_key_path,private_key_path,crt_file_path){
	...
	var show_children_list = function(){
		raschi_data.find({connection : true},function(err,childs){
			for(var i=0;i<childs.list;i++){
				console.log(childs[i].id + "is connected now");
				var child_info = new child_connection(childs[i].id,true).toString();
				th.message_parse(child_info);
			}
		});
	};
