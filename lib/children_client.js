...
var children_client = function(public_key_path,private_key_path,crt_file_path){
	...
	var tls_client = function(socket,guid){
		...
		this.socket.on("connect",function(ex){
			...
			var child_info = new child_connection();
			child_info.addChildInfo(socket_this.guid,true);
			cinfostr = child_info.toString();
			th.message_parse(cinfostr);
		});	
		...
		this.socket.on("close",function(ex){
			...
			connection = false;
			var child_info = new child_connection();
			child_info.addChildInfo(socket_this.guid,false);
			cinfostr = child_info.toString();
			th.message_parse(cinfostr);
			th.delete_children_client(socket_this.guid);
		});
	...
	/**
	 * tls_clientを削除して接続情報を更新
	 */
	this.delete_children_client = function(guid){
		delete tls_clients[guid];	
		raschi_data.update({id : guid},{ $set : {connection : false}}, function(err){
	...
	var register_client_with_mongodb = function(obj){
		var item = new raschi_data(
				{name:obj["name"],id:obj["guid"],connection:true}
				);
	...
	var register_to_db = function(obj){
		raschi_data.find({},function(err,docs){
			...
			for(var i=0,size =docs.length;i < size; ++i){
				if(docs[i].id == obj["guid"]) guid = docs[i].id;
			}
			if(guid == -1){
				register_client_with_mongodb(obj);
			} else {
				raschi_data.update({id : guid},{ $set: {connection  : true } },function(err){
					if(err) console.log("mongodb error" + err);
				});
			}
		});
	...
	var start_udp_server = function(){
		try{
			...
			if(ex != "Socket is already bound"){
				console.log("UDP Server Error: " + ex);
			}
		}
	};
	...
	var run_system_command = function(str){
		obj = JSON.parse(str);
		obj.value.message.forEach(function(value){
			switch(value){
				case 'start_udp':
					start_udp_server();
					break;
				case 'show_children_list':
					show_children_list();
					break;
			}
		});
	...
	var show_children_list = function(){
		var child_info = new child_connection();
		raschi_data.find({},function(err,childs){
			for(var i=0;i<childs.length;i++){
				child_info.addChildInfo(childs[i].id,childs[i].connection);
			}
			th.message_parse(child_info.toString());
		});
	};
			...
			search_name_from_db(guid,function(name){
				...
				var obj = {
					...
					"value": {
						...
						"error":error_obj,
						"result" : undefined,
						"client" : json_obj.message.value.client
					}
				};
	 ...
	 */
	this.start = function(){
		raschi_data.update({},{$set : {connection : false}},function(err){
			if(err) console.log("Erro in mongodb");
			listen_tcp_server();
		});
	};
};
