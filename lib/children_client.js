...
var children_client = function(public_key_path,private_key_path,crt_file_path){
	...
	var tls_client = function(socket,guid){
		...
		this.socket.on("error",function(ex){
			console.log("[CHILDREN CLIENT] [TLS CLINET]  " + ex.stack);
		});
		...
		this.socket.on("connect",function(ex){
			console.log("[CHILDREN CLIENT] [TLS CLINET]  Open Connection " + socket_this.guid);
		});	
		...
		this.socket.on("close",function(ex){
			console.log("[CHILDREN CLIENT] [TLS CLINET] Close Connection to " + socket_this.guid);
			connection = false;
		});
		...
		this.socket.on('data',function(str){
			console.log("[CHILDREN CLIENT] [TLS CLIENT] Message recived:" + str);
	 ...
	 */
	var search_name_from_db = function(guid){
		raschi_data.find({},function(err,docs){
			var name = "";
			for(var i=0;size = docs.length; i< size; ++i){
	...
	udp_server.on("message",function(msg,rinfo){
		console.log("[CHILDREN CLIENT] [UDP SERVER] message comming:");
	...
	var start_udp_server = function(){
		...
		console.log("[CHILDREN CLIENT] [UDP SERVER] OPEN UDP SERVER");
	};
	...
	var connect_to_mpserver = function(){
		tcp_client.connect({port:tcp_client_port},function(){
			console.log("[CHILDREN CLIENT] [TCP CLIENT] CONNECT TO MPSERVER");
		});
	};
	...
	var tcp_server = new net.Server(function(socket){
		console.log("[CHILDREN CLIENT] [TCP SERVER] CONNECT CLIENT");
		connect_to_mpserver();
	...
	var listen_tcp_server = function(){
		tcp_server.listen(tcp_server_port,function(){
			console.log("[CHILDREN CLIENT] [TCP SERVER] SERVER LISTEN START");
		});
	};
	...
	var connect_to_children = function(str,client_info){
		...
		register_to_db(obj);
		create_tlsclient(obj.guid,client_info);		
	};
	/**
	 * 子機に命令を送信する
	 *
	...
	var send_command_to_children = function(str){
		...
		if(!tls_clients[json_obj.id].write(message)){
			var error_obj = new Error("Connection Refused");
			var name = search_name_from_db(json_obj.id); 
			var obj = {
					"type" : "result",
					"value" {
						"id" : json_obj.id,
						"name" : name,
						"value" : {
							"functionName" : json_obj.function,
							"hasError" : true,
							"cancelled" : true,
							"error" : error_obj,
							"result" : undefined
