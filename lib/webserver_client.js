...
var net = require("net");
var system_command = require("./src/system_command.js"); 
/**
 * インスタンスを初期化します。
 * @constructor
...
var websocket_client = function(){
	...
	var save_key = function(obj){
		...
		if(obj.value.result == 0){
			...
			send_system_command(["start_udp","show_children_list"]);
		}else{
			process.exit(1);
		}
	};
	...
	var send_system_command = function(message){
		var command_message = new system_command("childrenclient",message).toString();
		send_to_mpserver(command_message);
	};
