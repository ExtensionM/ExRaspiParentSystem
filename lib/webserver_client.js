...
var websocket_client = function(){
	...
	this.ws_client.on('open',function(){
		...
		th.choose_send(function(str){
			th.create_send_message(str,function(message){
	...
	this.choose_send = function(callback){
	...
	this.create_send_message = function(str,callback){
		if(str == "auth"){
			fs.readFile(th.setting_file_path,function(err,data){
				obj = JSON.parse(data);
				obj.type = "auth";
	...
	this.check_login_result = function(obj){
		result = obj["result"] == 0 ? "LOGIN OK" : "LOGIN BAD";
		console.log("[WEB SERVER] [WEB SOCKET] [" + result + "]");
		// TODO ログインに失敗した時の処理
	};
