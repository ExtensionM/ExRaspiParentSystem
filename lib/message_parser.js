...
var message_parser = function(){
	...
	var message_server = new net.Server(function(socket){
		socket.on('data',function(str){
			console.log("[MESSAGE PARSER] [TCP SERVER] DATA RECIVED");
			check_message(str);
		});
	});
	...
	webclient_sender.on('error',function(error){console.log("[MESSAGE PARSER] [TCP SOCKET to WEBSERVER CLIENT] ERROR")});
	webclient_sender.on('close',function(){
		webclient_sender.setTimeout(500,function(){
			console.log("[MESSAGE PARSER] [TCP SOCKET] WebClient Sender Reconnecting...");
	...
	children_sender.on('error',function(error){console.log("[MESSAGE PARSER] [TCP SOCKET to CHILDREN CLINET] ERROR")});
	children_sender.on('close',function(error){
		children_sender.setTimeout(500,function(){
			console.log("[MESSAGE PARSER] [TCP SOCKET] Children Client Sedner Reconnecting....");
	...
	this.start = function(){
		...
		webclient_sender.connect(this.webclient_server_port);
		children_sender.connect(this.children_server_port);
	};
}
